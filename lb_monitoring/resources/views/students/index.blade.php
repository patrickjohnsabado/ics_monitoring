...
<head>
  ...
  <title>Lab Monitoring | Students</title>
            ...
            <section class="content">
              <div class="row">
                ...
                {!! Form::open(['action' => 'StudentController@store', 'method' => 'POST', 'enctype' => 'multipart/form-data']) !!}
                  ...
                  {{Form::file('excel_upload')}}
                    {{ Form::hidden('method', 'import') }}
                  ...
                  <div class="box">
                    <div class="box-header">
                      <form action="/students/create">
                        <button class="btn btn-primary" style="width: 10%"><i class="zmdi zmdi-plus"></i>Add student</button>
                      ...
                      <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                          @foreach ($students as $student)
                            <tr>
                                <td style="text-align: center;"> <input form="formDelete" type="checkbox" name="delete[]" value="{{ $student->id_number }}"></td>
                                <td style="text-align: center;">{{$no++}}</td>
                                <td style="text-align: center;">{{$student->id_number}}</td>
                                <td style="text-align: center;">{{$student->fullname}}</td>                                
                                <td style="text-align: center;">{{$student->email}}</td>
                                <td style="text-align: center;">{{\App\Setting::where('current','=',1)->first()->token }}</td>
                                
                                <td style="text-align: center;">
                                @foreach ($printing_expend as $exp)
                                  @if ($student->id_number == $exp->student_id)
                                      {{\App\Setting::where('current','=',1)->first()->token - $exp->printing_cost}}
                                  @endif
                                @endforeach
                                @foreach ($non_print as $np)
                                  @if ($student->id_number == $np->id_number)
                                      {{\App\Setting::where('current','=',1)->first()->token}}
                                  @endif
                                @endforeach
                                <td>

                                  <div class="table-data-feature">
                                    <button onclick="window.open('/students/{{ $student->id }}');" class="btn btn-info"><i class="fa fa-fw fa-user"></i> More</button>
                                        </div>
